...
function FVT_DID_List_Autobuild(~) 
    ...
    switch carmodel_folder
        ...
        case 'd21rwd-fdc2'
            DSFile_CarModel = DSFile_DIDModelD21;
        case 'd31x-fdc'
            DSFile_CarModel = DSFile_DIDModelD31X;
        otherwise
            DSFile_CarModel = DSFile_DIDModelD31L;
    end
    ...
    for i = 1:length(DSFile_DIDNum)
        if strcmp(DSFile_CarModel(i,1),'Y') && (strcmp(DSFile_DIDServiceRead(i,1),'Y') || strcmp(DSFile_DIDServiceWrite(i,1),'Y')) && ...
            (contains(DSFile_DIDFunClass(i,1),'VCU') || contains(DSFile_DIDFunClass(i,1),'BCM') || contains(DSFile_DIDFunClass(i,1),'TPM') || ...
             contains(DSFile_DIDFunClass(i,1),'APS') || contains(DSFile_DIDFunClass(i,1),'ZEV') || contains(DSFile_DIDFunClass(i,1),'FoxtronPi'))
    ...
    DIDNum_NotImplemented = setdiff(DSFile_UsedDIDNum, DID_Num_NoMissing);

    if isempty(DIDNum_NotImplemented)
        disp('All DIDs are implemented')
    else % ~isempty(DIDNum_NotImplemented)
        fprintf('DID not implemented:\n')
        for i = 1:length(DIDNum_NotImplemented)
            disp(DIDNum_NotImplemented(i,1))
        end
    end
