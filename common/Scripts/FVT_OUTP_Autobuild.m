...
%% Initial settings
ref_car_model = {'d31hawd-fdc2','d21awd-fdc2'};
...
q = questdlg({'Check the following conditions:', '1. Current folder common\Scripts?', ...
    ...
    out_name = strcat("VOUTP_",out_name);            
     ...
     for j = 1:length(k_value)
         ...
         elseif (j==2)
            ...
            elseif (data_unit == "int8")
                Arry_cal(Numb_Cal,4) = num2cell(-128);
            elseif (data_unit == "int16")
                Arry_cal(Numb_Cal,4) = num2cell(-32768);                
            else
                Arry_cal(Numb_Cal,4) = num2cell(0);
            end
            if (data_unit == "uint8")
           ...
           elseif (data_unit == "uint32")
                ...
                Arry_cal(Numb_Cal,8) = num2cell(0);
            elseif (data_unit == "int8")
                Arry_cal(Numb_Cal,5) = num2cell(127);
                Arry_cal(Numb_Cal,6) = cellstr(sign_unit);
                Arry_cal(Numb_Cal,7) = cellstr('N/A');
                Arry_cal(Numb_Cal,8) = num2cell(0);
            elseif (data_unit == "int16")
                Arry_cal(Numb_Cal,5) = num2cell(32767);
                Arry_cal(Numb_Cal,6) = cellstr(sign_unit);
                Arry_cal(Numb_Cal,7) = cellstr('N/A');
                Arry_cal(Numb_Cal,8) = num2cell(0);                  
            else 
...
function Copy_DD_to_other_car_models(car_model, Common_Scripts_path, arch_Path)
    project_path = char(strcat(Common_Scripts_path, '/../../', car_model));
    source_dd_Path = [arch_Path '/outp'];
    target_dd_Path = [project_path '/software/sw_development/arch/outp'];
