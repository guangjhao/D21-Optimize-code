function FVT_SWC_HALIN_CDD_Autobuild()
q = questdlg({'Check the following conditions:','1. Run project_start?',...
    '2. SWC_HALIN_CDD_type.slx has not been modified?'},...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TargetModel = run_SWC_HAL_Rx_5ms_sys %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cd(project_path);
...
tmpCell1 = extractBetween(SWC_arxml(Raw_start+h-1,1),'SWC_HALIN_CDD_type/','</PORT-PROTOTYPE-REF>');
...
for i = 1:length(SWC_Input_array)
    if ~startsWith(SWC_Input_array(i),'R_CAN')... 
        ...
        && ~startsWith(SWC_Input_array(i),'R_NVM')...
        && ~startsWith(SWC_Input_array(i),'R_BINP')...
        && ~startsWith(SWC_Input_array(i),'R_BHAL')...
        && ~startsWith(SWC_Input_array(i),'R_BSCP')...
        && ~startsWith(SWC_Input_array(i),'R_BTPMS_APS')...
        && ~startsWith(SWC_Input_array(i),'R_UWB')
        idx = idx + 1;
...
for i = 1:length(Else_Input_array)
    if i == 1 
    targetpos = get_param([TargetModel '/R_BHAL_CAN4_outputs_BHAL_CAN4_outputs_read'],'Position');
    block_x = targetpos(1);
...
HALIN_CDD_Info = {'CDD_Name','Input_Name','Output_Name';
                  ...
                  'R_HALINCDD_CSOP_CAN6BusOff','','';...
                  'R_HALINCDD_CSOP_TPMSSensorIDRegGetSta','','';
                  'R_HALINCDD_CSOP_ReadPedalZeroPosReq','','';
                  'R_HALINCDD_CSOP_ResetPedalZeroPosReq','','';
                  'R_HALINCDD_CSOP_GetVinCode','','';
                  };
...
cnt = 0;
cnt2 = 0;
for i = 2:length(HALIN_CDD_Info(:,1))
    BlockName = char(HALIN_CDD_Info(i,1));  
    cnt = cnt +1;    
    Blockpos = get_param([TargetModel '/' BlockName],'position');
...
idx = ~contains(string(CDD_cell(:,1)),{'HALINCDD';'R_UDSCDD_CSOP_DdmClearDTCNotification';'R_HALIN_IMU_CSOP_IMUGetSixAxisVal';'R_HALIN_IMU_CSOP_IMUGetOffsetVal';'R_NVMReadCDD_Inner_NVM_ReadOperation'});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%% run_SWC_HALIN_CDD_Rx_5ms_sys Level      %%%%%%%%%%%%
...
% R_HALINCDD_CSOP_GetVinCode
CDDModel = 'R_HALINCDD_CSOP_GetVinCode';
h = get_param([TargetModel '/' CDDModel],'Handle');
Simulink.BlockDiagram.createSubsystem(h);
h = get_param([TargetModel '/Subsystem'],'Handle');
P = get_param([TargetModel '/Subsystem'],'Position');
block_x = P(1);
block_y = P(2) +100;
block_w = block_x +700;
block_h = block_y +120;
...
% TargetModel = 'SWC_HAL_type/run_SWC_FDC_AppCallSleep_sys';
% open_system(TargetModel);
% BlockList = find_system(TargetModel,'SearchDepth','1');
% Function_x = 0;
% Function_y = - 100;
...
% TargetModel = 'SWC_HAL_type/P_WDGCDD';
% targetpos = get_param('SWC_HAL_type/run_SWC_FDC_UDS_10ms_sys','Position');
% block_x = targetpos(1);
% block_y = targetpos(2) +200;
...
% 
%% TargetModel = run_SWC_HALIN_CDD_IOEXPCDD_10ms_sys
TargetModel = 'SWC_HALIN_CDD_type/run_SWC_HALIN_CDD_IOEXPCDD_10ms_sys';
open_system(TargetModel);
BlockList = find_system(TargetModel,'SearchDepth','1');
Function_x = 0;
Function_y = - 100;
...
%% TargetModel = run_SWC_HALIN_CDD_Rx_10ms_sys
% TargetModel = 'SWC_HALIN_CDD_type/run_SWC_HALIN_CDD_Rx_10ms_sys';
% open_system(TargetModel);
% BlockList = find_system(TargetModel,'SearchDepth','1');
% Function_x = 0;
% Function_y = - 100;
...
%% Auto fix RUNNABLES
TargetModel = 'SWC_HALIN_CDD_type';
RUNNABLES_BlockList = find_system(TargetModel,'SearchDepth','1','BlockType','SubSystem');
idx = contains(RUNNABLE_array,{'run_SWC_HALIN_CDD_Rx_5ms';'run_SWC_HALIN_CDD_Rx_10ms';'run_SWC_HALIN_CDD_IOEXPCDD_10ms'});
RUNNABLE_array(idx) = [];
...
TargetSWC = 'SWC_HALIN_CDD_type';
open_system(TargetSWC);
cd(arch_Path);
FVT_SWC_DID_Autobuild(TargetSWC);
disp('SWC_HALIN_CDD_type Done!');
end
...
function Add_connect_line_between_Inport_and_Outport(Inputport_array,Outputport_array,TargetModel,warning_flg)
    ...
    if length(Inputport_array) == length(Outputport_array) && length(Outputport_array) == 1
        ...
        add_line(TargetModel,sourceport.Outport(1),targetport.Inport(1));
    elseif length(Inputport_array) < length(Outputport_array) && length(Inputport_array) == 1
        sourceport = get_param(char(Inputport_array),'PortHandles');
        for k = 1:length(Outputport_array)
            targetport = get_param(char(Outputport_array(k,1)),'PortHandles');
            add_line(TargetModel,sourceport.Outport(1),targetport.Inport(1),'autorouting','on');
        end
    else
