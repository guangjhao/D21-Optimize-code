...
% q = questdlg({'Check the following conditions:', '1. Do not run project_start.',...
%     '2. Do not generate any model.', '3. Change SWC_FDC.arxml file name to SWC_FDC_ORIGINAL.arxml.', '4. Current folder is Scripts?'},...
...
%%% ------ Cycle Tx ------
amount_cell_CYCLE = find(contains(FINAL_CELL_CAN_TX(:, 3),'PENDING'));
...
for i = 1 : length(allCAN_CycleUniquePeriods_cell)
     location_UniqueRunnable = find(contains(FINAL_CELL_CAN_TX_CYCLE(:, 2), allCAN_CycleUniquePeriods_cell(i)), 1, 'first');
...
for i = 1 : length(allCAN_CycleUniquePeriods_No10ms_cell)
     location_UniqueRunnable = find(contains(FINAL_CELL_CAN_TX_CYCLE_No_10ms(:, 2), allCAN_CycleUniquePeriods_No10ms_cell(i)), 1, 'first');
...
%%% ------ CE Tx ------
amount_cell_CE = find(contains(FINAL_CELL_CAN_TX(:, 3),'TRIGGERED-ON-CHANGE'));
