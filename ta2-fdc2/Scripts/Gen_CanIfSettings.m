...
for i = 1:length(Channel_list)
    ...
    for k = 1:length(DBC.Messages)
        ...
        if startsWith(MsgName,'NMm_FUSION')
            ...
            R_SG_APP{end,6} = num2str(DBC.MessageInfo(k).Extended);
            cnt = cnt +1;
            ...
            P_SG_APP{cnt,6} = num2str(DBC.MessageInfo(k).Extended);
        elseif any(strcmp(R_SG_APP(:,1),MsgName)) && ~strcmp(TxNode,'FUSION')
            ...
            R_SG_APP{h,6} = num2str(DBC.MessageInfo(k).Extended);
            % R_SG_APP{h,6} = TxNode;
...
for i = 1:length(R_SG_APP(:,1))
    ...
    % CAN type
    if strcmp(R_SG_APP(i,3),'CAN FD') && strcmp(R_SG_APP(i,6),'0')
        CanIfRxPduCanIdType = cellstr('STANDARD_FD_CAN');
    elseif strcmp(R_SG_APP(i,3),'CAN FD') && strcmp(R_SG_APP(i,6),'1')
        CanIfRxPduCanIdType = cellstr('EXTENDED_FD_CAN');
    elseif startsWith(R_SG_APP(i,1),'XCP_')
        CanIfRxPduCanIdType = cellstr('STANDARD_CAN');
    elseif strcmp(R_SG_APP(i,3),'J1939') || (strcmp(R_SG_APP(i,3),'CAN') && strcmp(R_SG_APP(i,6),'1'))
        CanIfRxPduCanIdType = cellstr('EXTENDED_NO_FD_CAN');
    else
...
for i = 1:length(P_SG_APP(:,1))
    ...
    % CAN type
    if strcmp(P_SG_APP(i,3),'CAN FD') && strcmp(P_SG_APP(i,6),'0')
        CanIfTxPduCanIdType = cellstr('STANDARD_FD_CAN');
    elseif strcmp(P_SG_APP(i,3),'J1939') || (strcmp(P_SG_APP(i,3),'CAN') && strcmp(P_SG_APP(i,6),'1'))
        CanIfTxPduCanIdType = cellstr('EXTENDED_CAN');
    elseif strcmp(P_SG_APP(i,3),'CAN FD') && strcmp(P_SG_APP(i,6),'1')
        CanIfTxPduCanIdType = cellstr('EXTENDED_FD_CAN');    
    else
    ...
    if startsWith(P_SG_APP(i,1),'XCP_') || startsWith(P_SG_APP(i,1),'Diag')
        ...
        if contains(tmpCell(h+1,1),'<VALUE>')
            ...
            NewString = ['                      <VALUE>' num2str(cnt) '</VALUE>'];
            tmpCell = [tmpCell(1:h);NewString;tmpCell(h+1:end)];
        end
    else
        h = find(contains(tmpCell,'/CanIfTxPduSourcePduID</DEFINITION-REF>'));
