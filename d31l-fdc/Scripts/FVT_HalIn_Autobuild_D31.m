function FVT_HalIn_Autobuild_D31()
%% Select Project
...
%         end
     %%
        ...
        % write DD file
        if contains(MsgName, {'ESC1', 'ESC5', 'ESC7'}) || contains(MsgName, {'FCM2', 'FCM3', 'FCM4', 'ABM1', 'BMS1', 'BMS6', 'CCU1', 'CCU2', 'Shifter', 'MCU_R1', 'MCU_F1'}) 
        ...
        if exist('Detect_AppMsg','var') && ~isempty(Detect_AppMsg) && ~isempty(Detect_APP_signal_can)
           ...
           if Autosar_flg 
            ...
            if contains(MsgName, {'ESC1', 'ESC5', 'ESC7'}) || contains(MsgName, {'FCM2', 'FCM3', 'FCM4', 'ABM1', 'BMS1', 'BMS6', 'CCU1', 'CCU2', 'Shifter', 'MCU_R1', 'MCU_F1'}) 
                ...
                DataID = char(cellstr(DBC.MessageInfo(cell2mat(RxMsgTable(j,1))).AttributeInfo(strcmp(DBC.MessageInfo(j).Attributes(:, 1), 'DataID_Hex')).Value));
                ...
                % Process CS from ESC
                if contains(MsgName, {'ESC1', 'ESC7'}) % non-CRC8 ckecksum
                    ...
                    portpos = get_param(muxport.Outport(1), 'Position');
                    srcT = 'simulink/Signal Routing/Goto';
                    dstT = [TargetModel '/' BlockName '3'];
                    block_x = portpos(1) + 100;
                    block_y = portpos(2) - 10;
                    block_w = block_x + 250;
                    block_h = block_y + 20;
                    goto = add_block(srcT, dstT, 'MakeNameUnique', 'on');
                ...
                else % CRC8 ckecksum    % 'ESC5', 'FCM2', 'FCM3', 'FCM4', 'ABM1', 'BMS1', 'BMS6', 'CCU1', 'CCU2', 'Shifter', 'MCU_R1', 'MCU_F1'
                    ...
                    for k = 1:length(Signal_match_array)
                        ...
                        if contains(SignalName, {'CRC8', 'Checksum', 'ChckSum', 'CS', 'Chksum'}) && ~contains(SignalName, {'ACC'})
                            EndByte = floor(Startbit / 8);
                            csbytepos = EndByte - NUM_BYTE + n + 1;
                        end
                    end                  
                ...
                if contains(MsgName_DD, {'ABM1', 'CCU1', 'CCU2', 'BMS1', 'BMS6', 'Shifter1'})
                    DD_cell2(DD_Index2(1), 8) = {'1'}; % Default during Running
                else
                    DD_cell2(DD_Index2(1), 8) = {'0'}; % Default during Running
                end

                % Check if the CS and RC from ESC are correct
                if contains(MsgName, {'ESC1', 'ESC7'}) % non-CRC8 ckecksum

                    BlockName = 'DDM_ESC_CS&RC_Comparison';
                    ...
                    block_w = block_x + 250;
                    block_h = block_y + 350;
                    srcT = 'FVT_lib/hal/DDM_ESC_CS&RC_Comparison';
                    dstT = [TargetModel '/' BlockName];
                    ...
                    if length(DataID) < 4
                        ...
                        for ii = 1:4 - length(DataID)
                            DataID = ['0' DataID];
                        end

                    end

                    DataID_H = ['0x' DataID(1:2)];
                    DataID_L = ['0x' DataID(3:4)];
                ...
                elseif contains(MsgName, {'FCM2', 'FCM3', 'FCM4', 'ABM1', 'BMS1', 'BMS6', 'CCU1', 'CCU2', 'Shifter', 'MCU_R1', 'MCU_F1'}) 
                    ...
                    BlockName = 'DDM_ESC5_CS&RC_Comparison';
                    targetblockpos = get_param(gotoport.Inport(1), 'Position');
                    block_x = targetblockpos(1) - 100;
                    block_y = targetblockpos(2) + 200 + ceil(str2double(DLC)) * 45/2;
                    block_w = block_x + 250;
                    block_h = block_y + 350;
                    srcT = 'FVT_lib/hal/DDM_ESC5_CS&RC_Comparison';
                    dstT = [TargetModel '/' BlockName];
                    hblock = add_block(srcT, dstT, 'MakeNameUnique', 'on');
                    set_param(hblock, 'position', [block_x, block_y, block_w, block_h]);
                    set_param(hblock, 'MsgCycleTime', msg_time_ms);
...
%                     end
                ...
                % Add RC goto block from ECU
                if (contains(MsgName, {'ESC1', 'ESC5', 'ESC7'}) && (contains(SignalName, 'RC') && contains(SignalName, MsgName) && ~contains(SignalName, 'CRC8'))) || contains(SignalName, 'ShifterRC') ...
                    || (contains(MsgName, {'FCM2', 'FCM3', 'FCM4', 'ABM1', 'BMS1', 'BMS6', 'CCU1', 'CCU2', 'MCU_R1', 'MCU_F1'}) && (contains(SignalName, 'RC') || contains(SignalName, 'LifeCount') || contains(SignalName, 'RolCnct') || contains(SignalName, 'RolCnt'))  && ~contains(SignalName, 'CRC8') && ~contains(SignalName, 'RCenter'))
