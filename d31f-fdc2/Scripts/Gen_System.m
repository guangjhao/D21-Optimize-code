...
for k = 1:length(Source_arxml)
    if strcmp(strip(char(Source_arxml(k)),'left'),'<P-PORT-PROTOTYPE>') && contains(Source_arxml(k+2),'CANInterface_ARPkg')
        cnt = cnt + 1;
        Port_SWC(cnt,1) = extractBetween(Source_arxml(k+1),'<SHORT-NAME>' ,'</SHORT-NAME>');
        Port_SWC(cnt,2) = cellstr([char(extractBetween(Source_arxml(k+2),'>' ,'<')) '/' char(extractBetween(Source_arxml(k+2),'/IF_' ,'<'))]);
        Port_SWC(cnt,3) = cellstr(['/' char(extractBetween(Source_arxml(k+2),'IF_' ,'_')) '/SignalGroup/' char(extractBetween(Source_arxml(k+2),'/IF_' ,'<'))]);
...
% Add FD_CONN1 in Port_SWC
Port_OTA = unique(DTCell(endsWith(DTCell(:,1),'_CONN1')));
for i = 1:length(Port_OTA)
    cnt = cnt+1;
    Channel = char(extractBetween(Port_OTA(i),'_','_SG'));
    Port_SWC(cnt,1) = cellstr(['P_' char(erase(Port_OTA(i),{'DT_';'SG_'}))]);
...
%% Modify Signal mapping for main SWC_HALOUT
...
for i = 1:length(Port_SWC(:,1))
    if strcmp(char(Port_SWC(i,1)),'R_HALINCDD') || strcmp(char(Port_SWC(i,1)),'R_HALOUTCDD') || strcmp(char(Port_SWC(i,1)),'R_TBOX1_Remote') 
        continue;
    end
...
%         Porttype = 'R-PORT-PROTOTYPE';
    ...
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['MAP_HALOUT' char(Port_SWC(i,1))];
    ...
    if endsWith(char(Port_SWC(i,1)),'_OTA1') || endsWith(char(Port_SWC(i,1)),'_CONN1')
        ...
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_VES_ARPkg/SWC_VES_type/P_CAN1_FD_BCM1
    else
        h = find(contains(tmpCell,'<CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">'),1,'first');
        OldString = extractBetween(tmpCell(h),'>','<');
        NewString = ['/Comp_' TargetECU_Abb '_ARPkg/Comp_' TargetECU_Abb '_main/SWC_HALOUT'];
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /Comp_FDC_ARPkg/Comp_FDC_main/SWC_HALOUT
        ...
        h = find(contains(tmpCell,'<CONTEXT-PORT-REF DEST="P-PORT-PROTOTYPE">'),1,'first');
        OldString = extractBetween(tmpCell(h),'>','<');
        NewString = ['/SWC_HALOUT_ARPkg/SWC_HALOUT_type/' char(Port_SWC(i,1))];
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_HALOUT_ARPkg/SWC_HALOUT_type/P_CAN1_FD_BCM1
    end
...
%% Modify Signal mapping for SWC_HALIN
% Template reused
FirstMessage = boolean(1);

for i = 1:length(Port_SWC(:,1))
    if strcmp(char(Port_SWC(i,1)),'R_HALINCDD') || strcmp(char(Port_SWC(i,1)),'R_HALOUTCDD')...
            || strcmp(char(Port_SWC(i,1)),'R_ExtTrig_CGW') || strcmp(char(Port_SWC(i,1)),'R_TBOX1_Remote') 
        continue;
    end
    ...
    h = find(contains(tmpCell,'<SHORT-NAME>SignalMappings</SHORT-NAME>'),1,'first');
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['MAP_HALIN_' char(Port_SWC(i,1))];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % <SHORT-NAME>MAP_R_CAN4_FCM1</SHORT-NAME>
    ...
    h = find(contains(tmpCell,'<CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">'),1,'first');
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/Comp_' TargetECU_Abb '_ARPkg/Comp_' TargetECU_Abb '_main/SWC_HALIN'];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /Comp_FDC_ARPkg/Comp_FDC_main/SWC_HALIN
    ...
    h = find(contains(tmpCell,'<CONTEXT-PORT-REF DEST="P-PORT-PROTOTYPE">'),1,'first');
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/SWC_HALIN_ARPkg/SWC_HALIN_type/' char(Port_SWC(i,1))];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_HALIN_ARPkg/SWC_HALIN_type/P_CAN1_FD_BCM1
    ...
    idx_DTCell = find(contains(DTCell(:,1),char(extractAfter(Port_SWC(i,3),'SignalGroup/'))));
    FirstSignal = boolean(1);
    for m = 1:length(idx_DTCell)
        Raw_start = find(contains(Template,'<SENDER-REC-RECORD-ELEMENT-MAPPING>'),1,'first');
        Raw_end = find(contains(Template,'</SENDER-REC-RECORD-ELEMENT-MAPPING>'),1,'first');
