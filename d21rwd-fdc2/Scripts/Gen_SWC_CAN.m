function Gen_SWC_CAN(Channel_list,MsgLinkFileName,TargetECU,DBCSet,RoutingTable)
project_path = pwd;
...
% Modify run_SWC_HALIN_Rx_5ms
Runnable_start = find(contains(HALIN_arxml,'<SHORT-NAME>run_SWC_HALIN_Rx_5ms'))-1;
h = find(contains(HALIN_arxml(:,1),'</RUNNABLE-ENTITY>'));
Runnable_end = min(h(h > Runnable_start));

% Delete drparg_CANxxx in <DATA-RECEIVE-POINT-BY-ARGUMENTS>
h  = Runnable_start + find(contains(HALIN_arxml(Runnable_start:Runnable_end),'<SHORT-NAME>drparg_CAN'))-2;
Var_numb = sum(contains(HALIN_arxml(Runnable_start:Runnable_end),'<SHORT-NAME>drparg_CAN'));

for j = 1:Var_numb
    % VARIABLE-ACCESS
    VarAcess_raw_start = h(j);
    h1 = find(contains(HALIN_arxml,'</VARIABLE-ACCESS>'));
    VarAcess_raw_end = min(h1(h1 > VarAcess_raw_start));
    HALIN_arxml(VarAcess_raw_start:VarAcess_raw_end) = {'xxx'};
end

% Delete <DATA-RECEIVE-POINT-BY-ARGUMENTS>
% Data Receive point by argument delete if no any drparg_xxx
Data_Receive_point_check = any(contains(HALIN_arxml(Runnable_start:Runnable_end),'<SHORT-NAME>drparg'));
if ~Data_Receive_point_check
    Data_Receive_point_raw_start = Runnable_start + find(contains(HALIN_arxml(Runnable_start:Runnable_end),'<DATA-RECEIVE-POINT-BY-ARGUMENTS>')) -1;
    Data_Receive_point_raw_end = Runnable_start + find(contains(HALIN_arxml(Runnable_start:Runnable_end),'</DATA-RECEIVE-POINT-BY-ARGUMENTS>'))-1;
    HALIN_arxml(Data_Receive_point_raw_start:Data_Receive_point_raw_end) = {'xxx'};
end
...
%% Output HALOUT_arxml
cd([project_path '/documents/ARXML_output'])
fileID = fopen( 'FVT_HALIN_ORIGINAL.arxml','w');
...
for i = 1:Runnable_numb
    ...
    if ~Runnable_check && ~strcmp(DeleteRunnable_ms,'10ms')        
