...
%     end
    ...
    ar = arxml.importer(k);
    cd(arch_path);
    createCompositionAsModel(ar, ['/Comp_' TargetECU_Abb '_ARPkg/Comp_' TargetECU_Abb '_main'], 'ModelPeriodicRunnablesAs', 'FunctionCallSubsystem','DataDictionary', 'APPTypes.sldd');
...
if code_generate
    ...
    filePaths = fullfile(arch_path, {'SWC_FDC_type.slxc', 'SWC_CDD_type.slxc', 'SWC_VES_type.slxc',...
        'SWC_UDS_type.slxc', 'SWC_IMU_type.slxc', 'SWC_UART_type.slxc','SWC_DID_type.slxc','SWC_APS_type.slxc','SWC_IOEXP_type.slxc','SWC_CGW_type.slxc'});
    folderPaths = fullfile(arch_path, {'slprj', 'SWC_CDD_type_autosar_rtw', 'SWC_FDC_type_autosar_rtw',...
        'SWC_UDS_type_autosar_rtw', 'SWC_VES_type_autosar_rtw', 'SWC_IMU_type_autosar_rtw', 'SWC_UART_type_autosar_rtw', 'SWC_DID_type_autosar_rtw', 'SWC_APS_type_autosar_rtw', 'SWC_IOEXP_type_autosar_rtw', 'SWC_CGW_type_autosar_rtw'});
    for i = 1:length(filePaths)
        if exist(filePaths{i}, 'file')
            delete(filePaths{i});
        end
    ...
    IOEXP_code_path = [arch_path '\SWC_IOEXP_type_autosar_rtw'];
    CGW_code_path = [arch_path '\SWC_CGW_type_autosar_rtw'];
    ...
    mkdir(RAW_CODE_path,'SWC_IOEXP_type_autosar_rtw');
    mkdir(RAW_CODE_path,'SWC_CGW_type_autosar_rtw');
    ...
    % IOEXP
    files = dir([IOEXP_code_path '\*.c']);

    for i = 1:length(files)
        copyfile([files(i).folder '\' files(i).name],[RAW_CODE_path '\SWC_IOEXP_type_autosar_rtw\' files(i).name])
    end

    files = dir([IOEXP_code_path '\*.h']);
    ...
    % CGW
    files = dir([CGW_code_path '\*.c']);

    for i = 1:length(files)
        copyfile([files(i).folder '\' files(i).name],[RAW_CODE_path '\SWC_CGW_type_autosar_rtw\' files(i).name])
    end

    files = dir([CGW_code_path '\*.h']);
