function FVT_SWC_HALOUT_Autobuild()
q = questdlg({'Check the following conditions:','1. Run project_start?',...
    '2. SWC_HALOUT_type.slx has not been modified?'},...
...
SWC_Input_array = strings(length(tmpCell1),1);
...
% PORT-PROTOTYPE-REF
h = find(contains(SWC_arxml(Raw_start:Raw_end,1),'<PORT-PROTOTYPE-REF'));
tmpCell1 = extractBetween(SWC_arxml(Raw_start+h-1,1),'SWC_HALOUT_type/','</PORT-PROTOTYPE-REF>');
...
for i = 1:length(SWC_Input_array)
    if ~startsWith(SWC_Input_array(i),'R_CAN')... 
        ...
        && ~startsWith(SWC_Input_array(i),'IRV_ms')...
		&& ~startsWith(SWC_Input_array(i),'IRV_E2E_CAN')...
        && ~startsWith(SWC_Input_array(i),'R_DKC')...
        && ~startsWith(SWC_Input_array(i),'R_BINP')...
        && ~startsWith(SWC_Input_array(i),'R_BSCP')
        idx = idx + 1;
...
ElseOutport_array = {'E2E_CAN3_VCU1_Trigger';'E2E_CAN3_VCU1_toNIDEC_Trigger';'E2E_CAN4_VCU1_Trigger'};
...
%                   'R_HALINCDD_CSOP_GetSWID','','u8_SWID_33byte';...
...
for i = 1:length(HALOUT_CDD_cell)
    ...
    if i == 1 
    targetpos = get_param([TargetModel '/' char(Else_Output_array(end,1))],'Position');
    block_x = targetpos(1);
    block_y = targetpos(2)+200;
    else
...
for i = 1:length(HALOUT_APS_cell(:,1))
    ...
    if i == 1 
    targetpos = get_param([TargetModel '/IRV_u8_ApsOutSetVehSpdTOCmd_write'],'Position');
    block_x = targetpos(1) + 3000;
    block_y = targetpos(2);
    else
...
% R_IOExpOUT_CDD_CSOP_IOExpSetVal
CDDModel = 'R_IOExpOUT_CDD_CSOP_IOExpSetVal';
h = get_param([TargetModel '/' CDDModel],'Handle');
Simulink.BlockDiagram.createSubsystem(h);
h = get_param([TargetModel '/Subsystem'],'Handle');
set_param(h,'Name',CDDModel);

% R_HALOUTCDD_CSOP_TPMSSensorIDRegSetResults
CDDModel = 'R_HALOUTCDD_CSOP_TPMSSensorIDRegSetResults';
h = get_param([TargetModel '/' CDDModel],'Handle');
Simulink.BlockDiagram.createSubsystem(h);
h = get_param([TargetModel '/Subsystem'],'Handle');
set_param(h,'Name',CDDModel);

% R_DIDWriteCDD_CSOP_DIDSet_NVMPedalZP
CDDModel = 'R_DIDWriteCDD_CSOP_DIDSet_NVMPedalZP';
h = get_param([TargetModel '/' CDDModel],'Handle');
Simulink.BlockDiagram.createSubsystem(h);
h = get_param([TargetModel '/Subsystem'],'Handle');
set_param(h,'Name',CDDModel);

% R_HALOUTCDD_CSOP_ResetPedalZeroPosDone
CDDModel = 'R_HALOUTCDD_CSOP_ResetPedalZeroPosDone';
h = get_param([TargetModel '/' CDDModel],'Handle');
Simulink.BlockDiagram.createSubsystem(h);
h = get_param([TargetModel '/Subsystem'],'Handle');
set_param(h,'Name',CDDModel);
...
% R_J1939_CAN1_FD1_CE100_1_CSOP_Write_CAN1_FD1_CE100_0
% R_J1939_CAN1_FD1_CE100_1_CSOP_Write_CAN1_FD1_CE100_1
CDDModel = {'R_J1939_CAN1_FD1_CE100_0_CSOP_Write_CAN1_FD1_CE100_0';'R_J1939_CAN1_FD1_CE100_1_CSOP_Write_CAN1_FD1_CE100_1'};
...
HALOUT_CDD_Info = {'CDD_Name','Inputport_Name','Input_Name','Trigger_Name';...
                   ...
                   'R_UDSCDD_CSOP_DdmRestartOperationCycle','','','VOUTP_EnterNewOperCyc_flg';...
                   'R_HALIN_IMU_CSOP_IMUSetSta','u8_IMUEnaCmd','','';...
                   'R_HALOUTCDD_CSOP_SetAcoreRebootCmdACK','','','VOUTP_AcoreRebootCmdACK_flg';...
                   'R_HALOUTCDD_CSOP_SetHazardIND','','VOUTP_HazardIND_flg/uint8','';...
                   'HALOUT_AFTER',{'J1939_CAN1_FD1_CE100_0_Trigger';'J1939_CAN1_FD1_CE100_1_Trigger'},{'J1939_CAN1_FD1_CE100_0_Trigger';'J1939_CAN1_FD1_CE100_1_Trigger'},'trig_HAL_OUTAFTER_50_1';...
                   ...
                   'R_HALOUTCDD_CSOP_TPMSSensorIDRegSetResults','','','VOUTP_TPMSSensorIDRegStart_flg';...
                   'R_IOExpOUT_CDD_CSOP_IOExpSetVal','u8_IVIRstCmd','VOUTP_IVIRST_flg/uint8','VOUTP_IVIRST_flg/DetectChange';...
                   'R_HALOUTCDD_CSOP_ResetPedalZeroPosDone','','','VOUTP_ResetPedalZeroPosStart_flg/DetectIncrease';...
                   'R_DIDWriteCDD_CSOP_DIDSet_NVMPedalZP','','',''};
...
CDDModel_array = ["R_J1939_CAN1_FD1_CE100_0_CSOP_Write_CAN1_FD1_CE100_0";"R_J1939_CAN1_FD1_CE100_1_CSOP_Write_CAN1_FD1_CE100_1"];
BlockName = 'Enable';
...
% R_UDSCDD_CSOP_DdmRestartOperationCycle (no input, trigger)
lastblock = get_param([TargetModel '/' CDDModel],'Position');
lastblock_y = lastblock(4);
CDDModel = 'R_UDSCDD_CSOP_DdmRestartOperationCycle';
block_x = lastblock_x;
...
% R_HALOUTCDD_CSOP_SetHazardIND (trigger)
lastblock = get_param([TargetModel '/' CDDModel],'Position');
lastblock_y = lastblock(4);
CDDModel = 'R_HALOUTCDD_CSOP_SetHazardIND';
block_x = lastblock_x;
block_y = lastblock_y + 150;
block_w = lastblock(3);
block_h = block_y + 120;
...
% R_IOExpOUT_CDD_CSOP_IOExpSetVal (input, trigger)
lastblock = get_param([TargetModel '/' CDDModel],'Position');
lastblock_y = lastblock(4);
CDDModel = 'R_IOExpOUT_CDD_CSOP_IOExpSetVal';
block_x = lastblock_x;
...
% R_HALOUTCDD_CSOP_TPMSSensorIDRegSetResults (trigger)
lastblock = get_param([TargetModel '/' CDDModel],'Position');
lastblock_y = lastblock(4);
CDDModel = 'R_HALOUTCDD_CSOP_TPMSSensorIDRegSetResults';
block_x = lastblock_x;
...
% R_DIDWriteCDD_CSOP_DIDSet_NVMPedalZP (trigger)
lastblock = get_param([TargetModel '/' CDDModel],'Position');
lastblock_y = lastblock(4);
CDDModel = 'R_DIDWriteCDD_CSOP_DIDSet_NVMPedalZP';
block_x = lastblock_x;
...
% R_HALOUTCDD_CSOP_ResetPedalZeroPosDone (trigger)
lastblock = get_param([TargetModel '/' CDDModel],'Position');
lastblock_y = lastblock(4);
CDDModel = 'R_HALOUTCDD_CSOP_ResetPedalZeroPosDone';
block_x = lastblock_x;
...
TargetModel = 'SWC_HALOUT_type/run_SWC_HALOUT_AppCallSleep_sys';
open_system(TargetModel);
BlockList = find_system(TargetModel,'SearchDepth','1');
Function_x = 0;
Function_y = - 100;
...
TargetModel = 'SWC_HALOUT_type/P_WDGCDD';
targetpos = get_param('SWC_HALOUT_type/run_SWC_HALOUT_UDS_10ms_sys','Position');
block_x = targetpos(1);
block_y = targetpos(2) +200;
...
TargetModel = 'SWC_HALOUT_type/run_SWC_HALOUT_Tx_10ms_sys';
open_system(TargetModel);
BlockList = find_system(TargetModel,'SearchDepth','1');
Function_x = 0;
Function_y = - 100;
...
%% Auto fix RUNNABLES
TargetModel = 'SWC_HALOUT_type';
RUNNABLES_BlockList = find_system(TargetModel,'SearchDepth','1','BlockType','SubSystem');
idx = contains(RUNNABLE_array,{'AppCallWDG';'AppCallSleep';'run_SWC_FDC_CDD_10ms';'run_SWC_HALOUT_Tx_10ms'});
RUNNABLE_array(idx) = [];
...
TargetSWC = 'SWC_HALOUT_type';
open_system(TargetSWC);
cd(arch_Path);
FVT_SWC_DID_Autobuild(TargetSWC);
cd(project_path);
disp('SWC_HALOUT_type Done!');
end
    ...
    for i =1:Inputport_Num
        ...
        if contains(Input_Name,'/')
            ...
            if contains(Input_Name,'/NOT/')
                ...
                if startsWith(Input_Name,'VOUTP_')
                    ...
                    h = add_block(srcT,dstT,'MakeNameUnique','on');
                    set_param(h,'position',[block_x,block_y,block_w,block_h]);
                    set_param(h,'OutputSignals',string(Input_Name),'ShowName', 'off');
                    sourceport = get_param(h,'PortHandles');
                    add_line(TargetModel,sourceport.Outport(1),targetport.Inport(1)); 
                    targetport = sourceport;
                    Input_Name = 'BOUTP_outputs';
                end
                BlockName = 'From';
...
function Add_CDD_trigger(TargetModel,CDDModel,Trigger_Name)
    ...
    if contains(Trigger_Name,'/DetectChange')
        ...
        if startsWith(Trigger_Name,'VOUTP_')
            BlockName ='bus_selector';
            srcT = 'simulink/Signal Routing/Bus Selector';
            dstT = [TargetModel '/' BlockName];
            block_x = targetpos(1) - 150;
            ...
            h = add_block(srcT,dstT,'MakeNameUnique','on');
            set_param(h,'position',[block_x,block_y,block_w,block_h]);
            set_param(h,'OutputSignals',string(Trigger_Name),'ShowName', 'off');
            sourceport = get_param(h,'PortHandles');
            add_line(TargetModel,sourceport.Outport(1),targetport.Inport(1)); 
            targetport = sourceport;
            Trigger_Name = 'BOUTP_outputs';
        end    
        BlockName = 'From';
        block_x = targetpos(1) - 685;
        block_y = targetpos(2) - 20;
    ...
    set_param(h,'GotoTag',Trigger_Name,'ShowName','off');    
    sourceport = get_param(h,'PortHandles');
    if isempty(targetport.Enable) && isempty(targetport.Trigger)
        add_line(TargetModel,sourceport.Outport(1),targetport.Inport(1),'autorouting','on');
    elseif ~isempty(targetport.Enable)
        add_line(TargetModel,sourceport.Outport(1),targetport.Enable(1),'autorouting','on');
    elseif ~isempty(targetport.Trigger)
