...
R_port_msg = extractBetween(Composition_arxml(h),'<SHORT-NAME>HALR_','</SHORT-NAME>');
% R_port_msg = extractAfter(R_port_msg,10);

h = find(contains(Composition_arxml,'<P-PORT-PROTOTYPE>')) + 1;
P_port_msg = extractBetween(Composition_arxml(h),'<SHORT-NAME>HALP_','</SHORT-NAME>');
        ...
        for k = Raw_start(i):Raw_end
            if ~strcmp(RoutingTable(k,4),'Invalid')
                ...
                if strcmp(RoutingTable(k,2),'Invalid')
                    ...
                    SourceMsgID = hex2dec(string(RoutingTable(h,3)));
                    TxMsgName = RoutingTable(h,5);
                    TxMsgID = hex2dec(string(RoutingTable(h,6)));
                else
                    SourceMsgName = RoutingTable(k,2);
                    SourceMsgID = hex2dec(string(RoutingTable(k,3)));
                    TxMsgName = RoutingTable(k,5);
                    TxMsgID = hex2dec(string(RoutingTable(k,6)));
                end
...
for i = 1:length(Channel_list)
    ...
    for k = 1:length(DBC.Messages)
        ...
        if IsDiag || startsWith(MsgName,'NMm_') || startsWith(MsgName,'Diag')
            continue
        end
        ...
        h = find(strcmp(extractAfter(R_port_msg,'_'),MsgName));
        h1 = find(strcmp(extractBefore(R_port_msg,'_'),Channel));
        h = intersect(h,h1);

        if ~isempty(h) && ~strcmp(DBC.MessageInfo(k).TxNodes,TargetECU) && strcmp(extractBefore(R_port_msg(h),'_'),Channel)
            MsgName = char(DBC.MessageInfo(k).Name);
...
for i = 1:length(Msg_idx)-1
    ...
    for k = 1:length(Signal_idx)
        ...
        if strcmp(ValueTable_R(Signal_idx(k),6),'BigEndian')
            ...
            ComBitPosition = cellstr(string(StartBit));
        end
...
for i = 1:length(Msg_idx)-1
    ...
    for k = 1:length(Signal_idx)
        ...
        if strcmp(ComTransferProperty_Msg,'PENDING')
            ComTransferProperty_Sig  = cellstr('TRIGGERED_ON_CHANGE'); % PDU PENDING Default, cannot select Sgnal ComTransferProperty
        elseif strcmp(ValueTable_P(Signal_idx(k),3),'OnChange') || strcmp(ValueTable_P(Signal_idx(k),3),'OnWriteWithRepetition')
            ComTransferProperty_Sig = cellstr('TRIGGERED_ON_CHANGE');
