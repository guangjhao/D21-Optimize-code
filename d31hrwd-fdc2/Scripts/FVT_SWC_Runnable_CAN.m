...
%% ------- Gather all CAN PDUs according to FVT_HALIN.arxml & FVT_HALOUT.arxml -------
fileID = fopen('FVT_HALOUT_ORIGINAL.arxml');
SWC_HALOUT_arxml_Updated = textscan(fileID,'%s', 'delimiter', '\n', 'whitespace', '');
fclose(fileID);
SWC_HALOUT_arxml_Updated = SWC_HALOUT_arxml_Updated{1};

location_P_CAN = find(contains(SWC_HALOUT_arxml_Updated,'<SHORT-NAME>P_CAN'));
amount_P_CAN = length(location_P_CAN);
P_port_cell = cell(amount_P_CAN,1);
for i = 1 : amount_P_CAN
    P_port_cell(i,1) = extractBetween(SWC_HALOUT_arxml_Updated(location_P_CAN(i,1),1), '<SHORT-NAME>P_', '</SHORT-NAME>');
end
...
fileID = fopen('FVT_HALIN_ORIGINAL.arxml');
SWC_HALIN_arxml_Updated = textscan(fileID,'%s', 'delimiter', '\n', 'whitespace', '');
fclose(fileID);
SWC_HALIN_arxml_Updated = SWC_HALIN_arxml_Updated{1};

location_R_CAN = find(contains(SWC_HALIN_arxml_Updated,'<SHORT-NAME>R_CAN'));
amount_R_CAN = length(location_R_CAN);
R_port_cell = cell(amount_R_CAN,1);
for i = 1 : amount_R_CAN
    R_port_cell(i,1) = extractBetween(SWC_HALIN_arxml_Updated(location_R_CAN(i,1),1), '<SHORT-NAME>R_', '</SHORT-NAME>');
end
...
FINAL_CELL_CAN_TX_CYCLE(:, end+1) = strcat('run_SWC_HALOUT_Tx_', FINAL_CELL_CAN_TX_CYCLE(:,2), 'ms');
FINAL_CELL_CAN_TX_CYCLE(:, end+1) = strcat('TE_HALOUT_', FINAL_CELL_CAN_TX_CYCLE(:,2), 'ms');
...
index_TE10ms = ismember(FINAL_CELL_CAN_TX_CYCLE(:, 5), 'TE_HALOUT_10ms');
...
%% ------ Add IRVs to FVT_HALOUT.arxml ------
location_IRVs = find(contains(SWC_HALOUT_arxml_Updated, '<EXPLICIT-INTER-RUNNABLE-VARIABLES>'), 1, 'first');
...
NewIRV_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_IRVs); NewIRV_cell; SWC_HALOUT_arxml_Updated(location_IRVs+1:end)];

%% ------- Update Runnable for 10ms cycle Tx CAN in FVT_HALOUT.arxml-------
location_Tx10ms_runnable = find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>run_SWC_HALOUT_Tx_10ms</SHORT-NAME>'));
location_Data_Send_Points_10ms = location_Tx10ms_runnable + find(contains(SWC_HALOUT_arxml_Updated(location_Tx10ms_runnable: end, 1), '<CAN-BE-INVOKED-CONCURRENTLY>false</CAN-BE-INVOKED-CONCURRENTLY>'), 1, 'first') - 1;
...
for i = 1 : size(FINAL_CELL_CAN_TX_CYCLE_Only_10ms, 1)
    ...
    NewCANTx10ms_cell(end+1, 1) = strcat('                          <PORT-PROTOTYPE-REF DEST="P-PORT-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/', FINAL_CELL_CAN_TX_CYCLE_Only_10ms(i, 6), '</PORT-PROTOTYPE-REF>');
...
NewCANTx10ms_cell(end+1, 1) = {'                  </DATA-SEND-POINTS>'};
NewCANTx10ms_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Data_Send_Points_10ms); NewCANTx10ms_cell; SWC_HALOUT_arxml_Updated(location_Data_Send_Points_10ms+1:end)];

location_Tx10ms_runnable = find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>run_SWC_HALOUT_Tx_10ms</SHORT-NAME>'));
location_Read_Local_Variables_10ms = location_Tx10ms_runnable + find(contains(SWC_HALOUT_arxml_Updated(location_Tx10ms_runnable: end, 1), '<READ-LOCAL-VARIABLES>'), 1, 'first') - 1;
...
for i = 1 : size(FINAL_CELL_CAN_TX_CYCLE_Only_10ms, 1)
    ...
    NewIRVRead10ms_cell(end+1, 1) = strcat('                        <LOCAL-VARIABLE-REF DEST="VARIABLE-DATA-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/SWC_HALOUT_type_IB/', FINAL_CELL_CAN_TX_CYCLE_Only_10ms(i, 8), '</LOCAL-VARIABLE-REF>');
    NewIRVRead10ms_cell(end+1, 1) =       {'                      </ACCESSED-VARIABLE>'};
    NewIRVRead10ms_cell(end+1, 1) =       {'                    </VARIABLE-ACCESS>'};    
end
NewIRVRead10ms_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Read_Local_Variables_10ms); NewIRVRead10ms_cell; SWC_HALOUT_arxml_Updated(location_Read_Local_Variables_10ms+1:end)];


%% ------- Add Runnable for Non-10ms cycle Tx CAN in FVT_HALOUT.arxml -------
...
location_Runnables = find(contains(SWC_HALOUT_arxml_Updated, '<RUNNABLES>'), 1, 'first');
...
for i = 1 : size(allCAN_CycleTxRunnables_No10ms_cell,1)
    ...
    for k = index_start : index_end
        ...
        NewRunnable_cell(end+1, 1) = strcat('                          <PORT-PROTOTYPE-REF DEST="P-PORT-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/', FINAL_CELL_CAN_TX_CYCLE_No_10ms(k, 6), '</PORT-PROTOTYPE-REF>');
    ...
    for y = index_start : index_end
        ...
        NewRunnable_cell(end+1, 1) =       {'                      <ACCESSED-VARIABLE>'};
        NewRunnable_cell(end+1, 1) = strcat('                        <LOCAL-VARIABLE-REF DEST="VARIABLE-DATA-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/SWC_HALOUT_type_IB/', FINAL_CELL_CAN_TX_CYCLE_No_10ms(y, 8), '</LOCAL-VARIABLE-REF>');
        NewRunnable_cell(end+1, 1) =       {'                      </ACCESSED-VARIABLE>'};
        NewRunnable_cell(end+1, 1) =       {'                    </VARIABLE-ACCESS>'};
    end
...
NewRunnable_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Runnables); NewRunnable_cell; SWC_HALOUT_arxml_Updated(location_Runnables+1:end)];

%%% ------ Add time event for each Tx runnable according to Tx period ------
location_Events = find(contains(SWC_HALOUT_arxml_Updated, '<EVENTS>'), 1, 'first');
...
for i = 1 : size(allCAN_CycleTxRunnables_No10ms_cell, 1)
    ...
    NewTimeEvent_cell(end+1, 1) = strcat('                  <START-ON-EVENT-REF DEST="RUNNABLE-ENTITY">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/SWC_HALOUT_type_IB/', allCAN_CycleTxRunnables_No10ms_cell(i, 2), '</START-ON-EVENT-REF>');
...
NewTimeEvent_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Events); NewTimeEvent_cell; SWC_HALOUT_arxml_Updated(location_Events+1:end)];

%% ------- Re-write FVT_HALIN.arxml for 5ms Rx and FVT_HALIN.arxml CE CAN Tx Runnable -------
%%% ------ Add CAN Rx data aaccess ------
location_5ms_Runnable = find(contains(SWC_HALIN_arxml_Updated, '<SHORT-NAME>run_SWC_HALIN_Rx_5ms</SHORT-NAME>'), 1, 'first');
h = find(contains(SWC_HALIN_arxml_Updated(:,1),'</RUNNABLE-ENTITY>'));
Runnable_end = min(h(h > location_5ms_Runnable));
NewCANRx5ms_cell = cell(1, 1);
if ~any(contains(SWC_HALIN_arxml_Updated(location_5ms_Runnable:Runnable_end), '<DATA-RECEIVE-POINT-BY-ARGUMENTS>'))
    location_Data_Receive_Point = location_5ms_Runnable + find(contains(SWC_HALIN_arxml_Updated(location_5ms_Runnable:end, 1), '<CAN-BE-INVOKED-CONCURRENTLY>'), 1, 'first') - 1;
    NewCANRx5ms_cell(end+1, 1) =       {'                  <DATA-RECEIVE-POINT-BY-ARGUMENTS>'};
else
    location_Data_Receive_Point = location_5ms_Runnable + find(contains(SWC_HALIN_arxml_Updated(location_5ms_Runnable:end, 1), '<DATA-RECEIVE-POINT-BY-ARGUMENTS>'), 1, 'first') - 1;
end
...
for i = 1 : size(FINAL_CELL_CAN_RX, 1)
    ...
    NewCANRx5ms_cell(end+1, 1) = strcat('                          <PORT-PROTOTYPE-REF DEST="R-PORT-PROTOTYPE">/SWC_HALIN_ARPkg/SWC_HALIN_type/', FINAL_CELL_CAN_RX(i, 4), '</PORT-PROTOTYPE-REF>');
...
NewCANRx5ms_cell(1, :) = [];
SWC_HALIN_arxml_Updated = [SWC_HALIN_arxml_Updated(1:location_Data_Receive_Point); NewCANRx5ms_cell; SWC_HALIN_arxml_Updated(location_Data_Receive_Point+1:end)];

%%% ------ Add CAN Tx data aaccess ------
location_5ms_Runnable = find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>run_SWC_HALOUT_Tx_5ms</SHORT-NAME>'), 1, 'first');
location_Data_Send_Point = location_5ms_Runnable +  find(contains(SWC_HALOUT_arxml_Updated(location_5ms_Runnable:end, 1), '</DATA-RECEIVE-POINT-BY-ARGUMENTS>'), 1, 'first') - 1;
...
for i = 1 : size(FINAL_CELL_CAN_TX_CE, 1)
    ...
    NewCANTx5ms_cell(end+1, 1) = strcat('                          <PORT-PROTOTYPE-REF DEST="P-PORT-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/', FINAL_CELL_CAN_TX_CE(i, 4), '</PORT-PROTOTYPE-REF>');
...
NewCANTx5ms_cell(end+1, 1) =       {'                  </DATA-SEND-POINTS>'};
NewCANTx5ms_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Data_Send_Point); NewCANTx5ms_cell; SWC_HALOUT_arxml_Updated(location_Data_Send_Point+1:end)];
                        
%%% ------ Add IRV write ------
location_Write_Local_Variables = location_5ms_Runnable +  find(contains(SWC_HALOUT_arxml_Updated(location_5ms_Runnable:end, 1), '<WRITTEN-LOCAL-VARIABLES>'), 1, 'first') - 1;
...
for i = 1 : size(FINAL_CELL_CAN_TX_CYCLE, 1)
    ...
    NewIRVwrite_cell(end+1, 1) = strcat('                        <LOCAL-VARIABLE-REF DEST="VARIABLE-DATA-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/SWC_HALOUT_type_IB/', FINAL_CELL_CAN_TX_CYCLE(i, 8), '</LOCAL-VARIABLE-REF>');
    NewIRVwrite_cell(end+1, 1) =       {'                      </ACCESSED-VARIABLE>'};    
    NewIRVwrite_cell(end+1, 1) =       {'                    </VARIABLE-ACCESS>'};
end
NewIRVwrite_cell(1, :) = [];
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Write_Local_Variables); NewIRVwrite_cell; SWC_HALOUT_arxml_Updated(location_Write_Local_Variables + 1 : end)];


%% ----- DID (HALOUT) -----
%%% ----- Delete original DIDList section -----

location_CSOP_DID_start = min(find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>scp_CSOP_DIDSet', 'IgnoreCase', false), 1, 'first') - 1, ...
    find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>scp_CSOP_DIDSet', 'IgnoreCase', false), 1, 'first') - 1);
location_CSOP_DID_end = max(find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>scp_CSOP_DIDSet', 'IgnoreCase', false), 1, 'last') + 6, ...
    find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>scp_CSOP_DIDSet', 'IgnoreCase', false), 1, 'last') + 6);
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_CSOP_DID_start - 1); SWC_HALOUT_arxml_Updated(location_CSOP_DID_end + 1:end)];
...
location_5ms_Runnable_start = find(contains(SWC_HALOUT_arxml_Updated, '<SHORT-NAME>run_SWC_HALOUT_Tx_5ms</SHORT-NAME>'), 1, 'first');
location_5ms_Runnable_end = find(contains(SWC_HALOUT_arxml_Updated, '<SYMBOL>run_SWC_HALOUT_Tx_5ms</SYMBOL>'), 1, 'first');
location_Data_Receive_Point = location_5ms_Runnable_start + find(contains(SWC_HALOUT_arxml_Updated(location_5ms_Runnable_start:location_5ms_Runnable_end, 1), '<SYNCHRONOUS-SERVER-CALL-POINT>'), 1, 'last') - 2;
NewDIDRxTx5ms_cell = cell(1, 1);
for i = 1 : size(DIDList_CSOPName, 1)
    if startsWith(DIDList_CSOPName(i),'CSOP_DIDSet')
        ...
        if string(DIDList_RWMode(i)) == 'W'
            NewDIDRxTx5ms_cell(end+1, 1) =       {'                        <CONTEXT-R-PORT-REF DEST="R-PORT-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/R_DIDReadCDD</CONTEXT-R-PORT-REF>'};
            ...
            NewDIDRxTx5ms_cell(end+1, 1) =       {'                        <CONTEXT-R-PORT-REF DEST="R-PORT-PROTOTYPE">/SWC_HALOUT_ARPkg/SWC_HALOUT_type/R_DIDWriteCDD</CONTEXT-R-PORT-REF>'};
...
SWC_HALOUT_arxml_Updated = [SWC_HALOUT_arxml_Updated(1:location_Data_Receive_Point); NewDIDRxTx5ms_cell; SWC_HALOUT_arxml_Updated(location_Data_Receive_Point+1:end)];

%% ----- DID (HALIN CDD) -----
%%% ----- Delete original DIDList section -----
cd ../documents/ARXML_output/
fileID = fopen('FVT_HALIN_CDD.arxml');
SWC_HALIN_CDD_arxml_Updated = textscan(fileID,'%s', 'delimiter', '\n', 'whitespace', '');
fclose(fileID);
SWC_HALIN_CDD_arxml_Updated = SWC_HALIN_CDD_arxml_Updated{1};
location_CSOP_DID_start = find(contains(SWC_HALIN_CDD_arxml_Updated, '<SHORT-NAME>scp_CSOP_DIDGet', 'IgnoreCase', true), 1, 'first') - 1;
location_CSOP_DID_end = find(contains(SWC_HALIN_CDD_arxml_Updated, '<SHORT-NAME>scp_CSOP_DIDGet', 'IgnoreCase', true), 1, 'last') + 6;
SWC_HALIN_CDD_arxml_Updated = [SWC_HALIN_CDD_arxml_Updated(1:location_CSOP_DID_start - 1); SWC_HALIN_CDD_arxml_Updated(location_CSOP_DID_end + 1:end)];
...
%%% ------ Add DID SERVER-CALL-POINT ------
location_5ms_Runnable_start = find(contains(SWC_HALIN_CDD_arxml_Updated, '<SHORT-NAME>run_SWC_HALIN_CDD_Rx_5ms</SHORT-NAME>'), 1, 'first');
location_5ms_Runnable_end = find(contains(SWC_HALIN_CDD_arxml_Updated, '<SYMBOL>run_SWC_HALIN_CDD_Rx_5ms</SYMBOL>'), 1, 'first');
location_Data_Receive_Point = location_5ms_Runnable_start + find(contains(SWC_HALIN_CDD_arxml_Updated(location_5ms_Runnable_start:location_5ms_Runnable_end, 1), '<SYNCHRONOUS-SERVER-CALL-POINT>'), 1, 'last') - 2;
NewDIDRxTx5ms_cell = cell(1, 1);
for i = 1 : size(DIDList_CSOPName, 1)
    if startsWith(DIDList_CSOPName(i),'CSOP_DIDGet')
        NewDIDRxTx5ms_cell(end+1, 1) =       {'                    <SYNCHRONOUS-SERVER-CALL-POINT>'};
        NewDIDRxTx5ms_cell(end+1, 1) = strcat('                      <SHORT-NAME>scp_', DIDList_CSOPName(i), '</SHORT-NAME>');
        NewDIDRxTx5ms_cell(end+1, 1) =       {'                      <OPERATION-IREF>'};
        if string(DIDList_RWMode(i)) == 'W'
            NewDIDRxTx5ms_cell(end+1, 1) =       {'                        <CONTEXT-R-PORT-REF DEST="R-PORT-PROTOTYPE">/SWC_HALIN_CDD_ARPkg/SWC_HALIN_CDD_type/R_DIDReadCDD</CONTEXT-R-PORT-REF>'};
            NewDIDRxTx5ms_cell(end+1, 1) = strcat('                        <TARGET-REQUIRED-OPERATION-REF DEST="CLIENT-SERVER-OPERATION">/Interface_DID_ARPkg/IF_DIDReadCDD/', DIDList_CSOPName(i), '</TARGET-REQUIRED-OPERATION-REF>');
        else % string(DIDList_RWMode(i)) == 'R'
            NewDIDRxTx5ms_cell(end+1, 1) =       {'                        <CONTEXT-R-PORT-REF DEST="R-PORT-PROTOTYPE">/SWC_HALIN_CDD_ARPkg/SWC_HALIN_CDD_type/R_DIDWriteCDD</CONTEXT-R-PORT-REF>'};
            NewDIDRxTx5ms_cell(end+1, 1) = strcat('                        <TARGET-REQUIRED-OPERATION-REF DEST="CLIENT-SERVER-OPERATION">/Interface_DID_ARPkg/IF_DIDWriteCDD/', DIDList_CSOPName(i), '</TARGET-REQUIRED-OPERATION-REF>');
        end
        NewDIDRxTx5ms_cell(end+1, 1) =       {'                      </OPERATION-IREF>'};
        NewDIDRxTx5ms_cell(end+1, 1) =       {'                      <TIMEOUT>0.0</TIMEOUT>'};
...
NewDIDRxTx5ms_cell(1, :) = [];
SWC_HALIN_CDD_arxml_Updated = [SWC_HALIN_CDD_arxml_Updated(1:location_Data_Receive_Point); NewDIDRxTx5ms_cell; SWC_HALIN_CDD_arxml_Updated(location_Data_Receive_Point+1:end)];

%% ------- Re-write FVT_HALIN.arxml -------
cd ../documents/ARXML_output/
fileID = fopen('FVT_HALIN.arxml','w');
for i = 1:length(SWC_HALIN_arxml_Updated(:,1))
    fprintf(fileID,'%s\n',char(SWC_HALIN_arxml_Updated(i,1)));
end
fclose(fileID);

%% ------- Re-write FVT_HALIN_CDD.arxml -------
fileID = fopen( 'FVT_HALIN_CDD.arxml','w');
for i = 1:length(SWC_HALIN_CDD_arxml_Updated(:,1))
    fprintf(fileID,'%s\n',char(SWC_HALIN_CDD_arxml_Updated(i,1)));
end
fclose(fileID);

%% ------- Re-write FVT_HALIN_CDD.arxml -------
fileID = fopen( 'FVT_HALOUT.arxml','w');
for i = 1:length(SWC_HALOUT_arxml_Updated(:,1))
    fprintf(fileID,'%s\n',char(SWC_HALOUT_arxml_Updated(i,1)));
end
fclose(fileID);

%% ------ Move original SWC_FDC.arxml file to backup place ------
% movefile( 'SWC_FDC_ORIGINAL.arxml', '../../../../SWC_FDC_ORIGINAL.arxml');
delete('FVT_HALIN_ORIGINAL.arxml')
delete('FVT_HALOUT_ORIGINAL.arxml')
