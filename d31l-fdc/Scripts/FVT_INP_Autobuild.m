...
q = questdlg({'Check tthe following conditions:','1. Run project_start?',...
    '2. Current folder arch?','3. Halin done?'},'Initial check','Yes','No','No');
if ~contains(q, 'Yes')
    return
end
arch_Path = pwd;
...
if strcmp(TargetNode,'FUSION')
    channel_list = {'CAN1','CAN2','CAN3','CAN4','CAN5','GPIO'};
...
%%
    ...
    if ~(strcmp(string(CANChannel),"CAN6"))
    ...
    if Numb_CANValid ~= 0
        Cal_CANValid = {'_inverse','_deblim','_debinc','_debdec','_ovrdflg','_ovrdval','_srvcflg','_srvcval'};
        srcT = 'simulink/Sources/In1';
...
%             set_param(h,'LinkStatus', 'inactive');
    ...
    Cal_CAN ={'_defval','_fftlim','_fftinc','_fftdec','_invalid','_setvalid','_ovrdflg','_ovrdval','_srvcflg','_srvcval'};
...
%             set_param(h,'LinkStatus', 'inactive');
            ...
            elseif strcmp(string(CANChannel),"CAN6")
