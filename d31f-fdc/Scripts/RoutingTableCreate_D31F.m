...
%     Channel = erase(Channel, '_');
    ...
    % d31f-fdc signal routing source is MCU_N, change signal name in
    % rouingtable if signal source is MCU_B
    [~, Car_Model] = fileparts(project_path);
%     if strcmp(Car_Model,'d31f-fdc')
%         idx = find(contains(CANSignalRouting_array(:,3),'MCU_B'));
%         CANSignalRouting_array(idx,2) = cellstr(string(CANSignalRouting_array(idx,2)) + '_Nidec');
%         CANSignalRouting_array(idx,3) = cellstr(strrep(string(CANSignalRouting_array(idx,3)),'B','N'));
%         if any(contains(CANSignalRouting_array(:,2),'W_Ver_Nidec'))
%             idx = contains(CANSignalRouting_array(:,2),'W_Ver_Nidec');
%             CANSignalRouting_array(idx,3) = cellstr(strrep(string(CANSignalRouting_array(idx,3)),'2','3'));
%         end
%     end
