...
for k = 1:length(Tx_Messages)
    ...
    % Ignore OTA and CONN ports
    if contains(MsgName,'_OTA1') || contains(MsgName,'_CONN1')
        continue
    end
...
R_CAN_Raw_start = find(contains(Target_arxml,'<SHORT-NAME>R_CAN'));
R_CAN_Raw_end = find(contains(Target_arxml,'</R-PORT-PROTOTYPE>'));
