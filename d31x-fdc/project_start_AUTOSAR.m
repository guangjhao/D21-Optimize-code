...
if ModelCreate
    ...
    cd(arch_path)
    if ~ModelUpdate
        FVT_SWC_Autobuild_D31F(Update_newDID)
    elseif Update_newDID
...
for i = 1:length(files)
    ...
    if contains(fileName, '_type.slx')
        modelName = strrep(strrep(fileName, '_type.slx', ''), 'SWC_', '');
        swcTypes{1, end+1} = modelName;
    end    
end
...
if code_generate
    ...
    for i = 1:length(swcTypes)
        buildmodel(['SWC_', swcTypes{i}, '_type'], arch_path);
    end
    ...
    %% Modify SWC_FDC_type.c SWC_FDC_type.h and SWC_DID_type.h
    Modify_c_and_h(RAW_CODE_path);
...
function delete_slxc_and_rtw(arch_path, swcTypes)
    filePaths = fullfile(arch_path, strcat('SWC_', swcTypes, '_type.slxc'));
    folderPaths = fullfile(arch_path, [{'slprj'}, strcat('SWC_', swcTypes, '_type_autosar_rtw')]);
...
function RAW_CODE_path = copy_files_to_fvt_app(project_path, arch_path, swcTypes)
    ...
    for i = 1:length(swcTypes)
        ...
        codePaths.(fieldName) = CopyFilestofvt_app_path(['SWC_', type, '_type_autosar_rtw'], arch_path, RAW_CODE_path);
    end

    FDC_code_path = codePaths.FDC_code_path;
...
function Modify_c_and_h(RAW_CODE_path)
    % add Watchdog
    ...
    if ~contains(file_content, insert_text)
        ...
        file_content = [file_content(1:search_idx + length(search_text) - 1) newline insert_text ...
    ...
    search_idx = strfind(file_content, search_text);
    insert_text = ['#include "FVT_API.h"' newline '#include <string.h>'];
    if ~contains(file_content, insert_text)
        if ~isempty(search_idx)
