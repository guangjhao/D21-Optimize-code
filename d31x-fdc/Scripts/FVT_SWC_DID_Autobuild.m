function FVT_SWC_DID_Autobuild(Update_newDID) 
    %% FVT_SWC_DID
    arch_Path = pwd;
    ...
    if Update_newDID
        ...
        DID_CDDSubsystem = [TargetModel '/DID_CDDSubsystem'];
        expand_subsystem(DID_GetSubsystem,TargetModel);
        expand_subsystem(DID_SetSubsystem,TargetModel);
        expand_subsystem(DID_CDDSubsystem,TargetModel); % old model
        % remove area
...
%     set_param(Out,'UseBusObject','on','BusObject',BlockName);
    ...
    for i =1: length(DID_CDDName)
        ...
        set_param(DIDNum,'value',char(extractAfter(DID_Input2(i),'_')),'ShowName','off');
        DIDNumport = get_param(DIDNum,'PortHandles');
        add_line(NewModel,DIDNumport.Outport,CDD_Wport.Inport(1));
...
%% Create DID Set subsystem (DIDWrite)
    ...
    for i =1: length(DID_CDDName)
        ...
        set_param(DIDNum,'value',char(extractAfter(DID_Input2(i),'_')),'ShowName','off');
        DIDNumport = get_param(DIDNum,'PortHandles');
        add_line(NewModel,DIDNumport.Outport,CDD_Rport.Inport(1));
        ...
        set_param(DIDLen,'value',char(extractAfter(DID_Input1(i),'_')),'ShowName','off');
        DIDLenport = get_param(DIDLen,'PortHandles');
        add_line(NewModel,DIDLenort.Outport,CDD_Rport.Inport(2));
