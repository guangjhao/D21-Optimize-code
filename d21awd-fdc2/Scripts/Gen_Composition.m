...
% Connectors for APP(HALIN)
for i = 1: length(R_SG_APP)
    tmpCell = Template; % initialize tmpCell

    h = find(contains(tmpCell,'<SHORT-NAME>RPortDeligationConnectorName</SHORT-NAME>'));
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['DELConn_HALIN_' erase(char(R_SG_APP(i)),'_SG')];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % <SHORT-NAME>DELConn_HALIN_CAN3_FD_VCU1</SHORT-NAME>

    h = find(contains(tmpCell,'<CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">'));
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/Comp_' TargetECU '_ARPkg/Comp_' TargetECU '_main/SWC_HALIN'];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /Comp_FDC_ARPkg/Comp_FDC_main/SWC_HALIN

    h = find(contains(tmpCell,'<TARGET-R-PORT-REF DEST="R-PORT-PROTOTYPE">'));
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/SWC_HALIN_ARPkg/SWC_HALIN_type/R_' erase(char(R_SG_APP(i)),'_SG')];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_HALIN_ARPkg/SWC_HALIN_type/R_CAN1_FCM3
...
for i = 1: length(P_SG_APP)
    ...
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['DELConn_HALOUT_' erase(char(P_SG_APP(i)),'_SG')];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % <SHORT-NAME>DELConn_HALOUT_CAN3_FD_VCU1</SHORT-NAME>

    if contains(SGName,'_OTA1') || contains(SGName,'_CONN1')
        h = find(contains(tmpCell,'DELConn_HALOUT_'));
        OldString = extractBetween(tmpCell(h),'>','<');
        NewString = ['DELConn_VES_' erase(char(P_SG_APP(i)),'_SG')];
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % <SHORT-NAME>DELConn_VES_CAN1_FD1_OTA1</SHORT-NAME>
	    ...
	    h = find(contains(tmpCell,'<CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">'));
	    OldString = extractBetween(tmpCell(h),'>','<');
	    NewString = ['/Comp_' TargetECU '_ARPkg/Comp_' TargetECU '_main/SWC_HALOUT'];
	    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /Comp_FDC_ARPkg/Comp_FDC_main/SWC_HALOUT
	
	    h = find(contains(tmpCell,'<TARGET-P-PORT-REF DEST="P-PORT-PROTOTYPE">'));
	    OldString = extractBetween(tmpCell(h),'>','<');
	    NewString = ['/SWC_HALOUT_ARPkg/SWC_HALOUT_type/P_' erase(char(P_SG_APP(i)),'_SG')];
	    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_HALOUT_ARPkg/SWC_HALOUT_type/P_CAN3_FD_VCU1
    end
