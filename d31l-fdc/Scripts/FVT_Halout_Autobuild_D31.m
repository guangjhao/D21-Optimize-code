    ...
    for j = 1:length(DBC.Messages)
        if ~contains(DBC.MessageInfo(j).Name,'CCP')...
            ...
            if IsLINMessage
                ...
                TxMsgTable(j,10) = cellstr(DBC.MessageInfo(j).AttributeInfo(strcmp(DBC.MessageInfo(j).Attributes(:,1),'DataID_Hex')).Value);
            end
            MsgCnt = MsgCnt + 1;
...
%         end
        ...
        if (strcmp(string(Channel),'CAN1') && (strcmp(string(MsgName),'FD_APS3') || strcmp(string(MsgName),'FD_VCU2') || strcmp(string(MsgName),'FD_VCU3') || strcmp(string(MsgName),'VMC_FCM1')))...
           || (strcmp(string(Channel),'CAN3') && (strcmp(string(MsgName),'FD_VCU2')))...
           || (strcmp(string(Channel),'CAN4') && strcmp(string(MsgName),'FD_VCU1'))          
            
            CRC_flg = boolean(1);
            
            CS_array = { 'CAN1' , 'FD_APS3' , 'APS3_checksum' ;...
                        'CAN1' , 'FD_VCU2' , 'CS_VCU2_CAN1' ;...
                        'CAN1' , 'FD_VCU3' , 'CS_VCU3_CAN1' ;...
                        'CAN1' , 'VMC_FCM1' , 'VMC_FCM1_Checksum' ;...
                        'CAN3' , 'FD_VCU2' , 'CS_VCU2_CAN1' ;...
                        'CAN4' , 'FD_VCU1' , 'CS_VCU1_CAN4'};
            
            RC_array = { 'CAN1' , 'FD_APS3' , 'APS3_LifeCount' ;...
                        'CAN1' , 'FD_VCU2' , 'RC_VCU2_CAN1' ;...
                        'CAN1' , 'FD_VCU3' , 'RC_VCU3_CAN1' ;...
                        'CAN1' , 'VMC_FCM1' , 'VMC_FCM1_LifeCount' ;...
                        'CAN3' , 'FD_VCU2' , 'RC_VCU2_CAN1' ;...
                        'CAN4' , 'FD_VCU1' , 'RC_VCU1_CAN4'};
            ...
            DD_cell(DD_Index_Msg(1),6) = {'flg'}; % Unit
            DD_cell(DD_Index_Msg(1),7) = {'N/A'}; % Enum Table
            if strcmp(string(Channel),'CAN1') && strcmp(string(MsgName),'FD_VCU2')...
                    || (strcmp(string(Channel),'CAN4') && strcmp(string(MsgName),'FD_VCU1')) % for E2E enable 
                DD_cell(DD_Index_Msg(1),8) = {'1'}; % Default during Running
            else
                DD_cell(DD_Index_Msg(1),8) = {'0'}; % Default during Running
            end
...
%             end
        ...
        elseif Autosar_flg
            Autosar_output_msg = char(Autosar_output_msg_CAN(contains(Autosar_output_msg_CAN,MsgName)));
            BlockName = 'bus_creator';
            block_x = original_x + 1400;
            block_y = original_y + 770;
            block_w = block_x + 20;
            block_h = block_y + 100*length(SignalName_raw);
            srcT = 'simulink/Signal Routing/Bus Creator';
            dstT = [TargetModel '/' BlockName];
            h = add_block(srcT,dstT);
            set_param(h,'Inputs',string(length(SignalName_raw)));
            set_param(h,'position',[block_x,block_y,block_w,block_h]);
            ...
            for k = 1:length(SignalName_raw)
                ...
                set_param(h,'position',[block_x,block_y,block_w,block_h]);
                set_param(h,'Gototag', [Channel '_' char(SignalName_raw(k))],'ShowName', 'off');
                sourceport = get_param(h,'PortHandles');
                g = add_line(TargetModel,sourceport.Outport(1),targetport.Inport(k));
                set_param(g,'name',[Channel '_' char(SignalName_raw(k))]);
